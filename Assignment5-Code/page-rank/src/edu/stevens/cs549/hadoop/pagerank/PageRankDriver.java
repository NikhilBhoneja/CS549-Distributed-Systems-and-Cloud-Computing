...
public class PageRankDriver {
	...
	//public static final String name_path = "input/nameInput.txt"; //for Testing nameInput
	public static final String name_path = "input/page-names/"; // for Testing page-names
	...
	static void init(String input, String output, int reducers)
		...
		//job.addCacheFile(URI.create(CACHE_DIR));
	...
	static void diff(String input1, String input2, String output, int reducers)
		...
		FileOutputFormat.setOutputPath(job, new Path("tempdiff1")); // Adds a temporary output folder "tempdiff"
		...
		if (job.waitForCompletion(true)) // Once the first job completes
		{
			...
			FileInputFormat.addInputPath(job1, new Path("tempdiff1")); 
			...
			System.out
					.print(job1.waitForCompletion(true) ? "Diff Job Completed"
							: "Diff Job Error");
			deleteDirectory("tempdiff1"); // Deletes the temporary directory "tempdiff"
		}
	...
	public static void composite(String input, String output, String interim1,
		 ...
		 */
		System.out.println("Nikhil Bhoneja(10473600)");
						...
						// is the input directory
		{
			...
			join(interim2,name_path,"join_output",reducers);
		...
		else // for even i, interim1 is the input directory
		{
			...
			counter++;
			// TODO: Modify this to output (vertex name, page rank) pairs instead of (vertex id, page rank)
			//join(interim1,"namesInput",output,reducers);
			join(interim1,name_path,"join_output",reducers);
			finish("join_output", output, reducers);
			summarizeResult(output);
		}
		System.out.println();
